...
const marks = Map<boolean, string>([[true, '■'], [false, '□']]);
...
export default function todoReduce(state: TodoState = initialState, action: MyAction): TodoState {
  ...
  function addTodo(state: TodoState, action: MyAction): TodoState {
    ...
    return Object.assign({}, state, { todos: state.todos.push(newTodo) });
  }

  function deleteTodo(state: TodoState, action: MyAction): TodoState {
    return Object.assign({}, state, { todos: state.todos.filter(item => item.id !== action.id) });
  }
